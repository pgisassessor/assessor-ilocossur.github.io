...
(function($) {
	...
	$(function() {
		$(document).on('click', 'a.page-scroll', function(event) {
			var $anchor = $(this);
			var addHeight = 150; // Default value
	
			// Check if the target section's ID is "functions"
			if ($($anchor.attr('href')).attr('id') === 'mandate') {
				addHeight = 80; // Override value for "functions" section
			}
	
			// Scroll to the target section with the specified offset
			$('html, body').stop().animate({
				scrollTop: $($anchor.attr('href')).offset().top - addHeight
			}, 600, 'easeInOutExpo');
	...
	function changeActive() {
		...
		$.each(menuItems, function(index, value) {
			...
			var docScrollAdjusted = docScroll + addHeight;
	
			if (docScrollAdjusted >= offsetSection) {
